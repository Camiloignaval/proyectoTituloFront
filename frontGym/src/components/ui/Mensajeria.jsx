...
export const Mensajeria = () => {
  ...
  const {
    info: { id_cargo, entrenador, id_usuario },
  ...
  useEffect(async () => {
    ...
    if (id_cargo === 3) {
      ...
      if (entrenador !== null) {
        setOpctionsDestinataries([
          { idUser: 1, nombre: "Administración" },
          { idUser: entrenador, nombre: "Entrenador" },
        ]);
      } else {
    ...
    if (id_cargo === 2) {
      try {
        const data = await fetchConToken(
          `${baseApi}api/personal/entrenados${id_usuario}`
        );
  ...
  const onSubmit = (data) => {
    const groceria = data.message
      .split(" ")
      .filter((p) => grocerias.includes(p));
    if (groceria.length > 0) {
      Swal.fire(
        "Atención",
        "Su mensaje incluye contenido no apropiado, favor borrar para continuar",
        "info"
      );
    } else {
      if (data.id_destinatario !== "Escoja destinatario" || isMassive) {
        dispatch(
          sendMessage({ cargo: id_cargo, id_usuario, ...data, isMassive })
        );
        reset();
        document.querySelector(".errorDestinatario").innerHTML = "";
      } else {
        document.querySelector(".errorDestinatario").innerHTML =
          "Favor escoger destinatario";
      }
    }
  };
  return (
          ...
          <div className="card" style={{ width: "800px" }}>
            <div className="card-header d-flex justify-content-between">
              ...
              <div className="btnMasivo ">
                {id_cargo !== 3 && (
                  <button
                    onClick={() => setIsMassive(!isMassive)}
                    type="submit"
                    className="btn massive_btn"
                  >
                ...
                {!isMassive && (
                  <div className="input-group form-group">
                    ...
                    >
                      ...
                      {opctionsDestinataries.map((o) => (
                        <option value={o?.idUser}>{o?.nombre}</option>
                      ))}
                    </select>
                    <p className="w-100 error errorDestinatario">
                      {errors.id_destinatario?.message}
                    </p>
                  </div>
                )}
                {/* subject */}
