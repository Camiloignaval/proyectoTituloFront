...
export const getAforoPorDia=  (date) => {
  return async () => {
    const resp = await fetchConToken(
      `http://localhost:4000/api/user/aforo${date}`
    )
...
export const getDaysOff= () => {
  return async () => {
    const resp = await fetchConToken(
      `http://localhost:4000/api/user/dayoff`
    )
    const body= await resp.json()
    return body
  }
...
export const getReserveHour= (id) => {
  return async (dispatch) => {
    const resp = await fetchConToken(
      `http://localhost:4000/api/user/reserve${id}`
    )
    const body= await resp.json()
    if(body.ok){
...
export const reserveHour=  (datos) => {
  return async (dispatch) => {
    const resp = await fetchConToken(
      `http://localhost:4000/api/user/reserve`,datos,'POST'
    )
    const data= await resp.json()
    if (data.ok) {
